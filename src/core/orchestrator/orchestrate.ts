...
import { createIntegrationOperations, type PullRequestInfo } from './integration-operations.ts';
...
export const createOrchestrator = (deps: OrchestrateDeps) => {
  ...
  const githubEffects = deps.config.github ? createGitHubEffects() : undefined;
   ...
   * Pull Request情報を生成
   *
   * WHY: IntegrationResultから完了タスクのサマリを含むPRタイトルと本文を作成
   */
  const buildPullRequestInfo = (completedTasks: Task[], userInstruction: string): PullRequestInfo => {
    const taskCount = completedTasks.length;
    const taskList = completedTasks.map(t => '- ' + t.id + ': ' + (t.summary || t.branch)).join('\n');
    return {
  ...
  const executeInstruction = async (
    ...
    try {
      ...
      if (deps.config.integration.method === 'pr' && !deps.config.github) {
        return createErr({
          type: 'PLANNING_ERROR',
          message: 'Integration method is "pr" but github config is not provided',
        });
      }
      ...
      if (completedTaskIds.length > 0 || failedTaskIds.length > 0) {
        ...
        if (integrationWorktreeInfo) {
          ...
          if (finalJudgement.isComplete) {
            ...
            // PR情報を生成
            const prInfo = buildPullRequestInfo(completedTasks, userInstruction);
            ...
            if (deps.config.integration.method === 'auto') {
            ...
            } else if (deps.config.integration.method === 'pr') {
              // PR作成を試行
              ...
              if (finalizeResult.ok && finalizeResult.val.method === 'pr') {
                console.log(`  ✅ Pull request created: ${finalizeResult.val.prUrl}`);
              } else {
                // PR作成失敗時はコマンドを出力
                console.log('  ⚠️  Pull request creation failed, please merge manually:');
                console.log(`\n     git checkout ${baseBranch} && git merge ${integrationBranch}\n`);
              }
            } else {
              // method === 'command'
