...
### fail-closed オプションの評価

#### 意義

「要件カバレッジ検証」の信頼性を担保するための保険。LLM呼び出し失敗時に欠陥タスクが通過するリスクを防止。

#### トレードオフ

| 観点 | fail-open (現状) | fail-closed |
|------|-----------------|-------------|
| 可用性 | 高（障害時も継続） | 低（障害で停止） |
| 安全性 | 低（欠陥タスク通過リスク） | 高 |
| 実用性 | 高 | 運用負荷増 |

#### 現実的な評価

- LLM呼び出し失敗は稀（Rate Limitやネットワーク障害時のみ）
- fail-closedにすると一時的な障害でタスク生成が完全停止
- 現状のfail-openは「とりあえず動かす」という実用的な判断
- 他のADR（013: スコアノイズ耐性、012: CLI進捗表示）の方が日常的な改善効果が大きい可能性

#### 結論

実装優先度は低い。必要性が明確になった場合（実際にfail-openで問題が発生した場合）に再検討。
