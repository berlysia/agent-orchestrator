...
有用性精査の結果、既存機能との重複が大きく、複雑性の増加に見合う価値がないと判断。
...
**ノイズ閾値による差分有意性判定** を採用予定だったが、却下。

## 却下理由

### 1. 既存機能との重複

現在の`isStagnated`関数（`planner-operations.ts:2507-2525`）で、`deltaThreshold`（デフォルト5）により5点未満の変動は既に「停滞」として処理される。

提案された`noiseThreshold`（デフォルト3）と比較:

| スコア変化 | deltaThreshold(5) | noiseThreshold(3) | 意思決定 |
|-----------|-------------------|-------------------|----------|
| +2点 | 停滞 | ノイズ | **同じ** |
| +4点 | 停滞 | 有意 | **同じ**（停滞判定でカバー） |
| -2点 | 停滞 | ノイズ | **同じ** |
| +6点 | 改善 | 有意 | **同じ** |

**ほぼ全てのケースで既存の`deltaThreshold`がカバー済み**。
...
|-----------------|------|
| 新しい型 | `ScoreDirection` |
| 新しい設定 | `noiseThreshold` |
| 新しい関数 | `isScoreChangeSignificant()`, `determineScoreDirection()` |
| return文変更 | 6箇所 |
| テスト追加 | 新規ファイル + 既存テスト更新 |

### 3. 実質的な効果が限定的

- **意思決定の変化**: なし（accept/replan/rejectは変わらない）
- **主な効果**: reasonの詳細化（「改善停滞」→「改善停滞（ノイズ範囲内）」）と`scoreDirection`追加のみ

### 4. ADR010との整合性

ADR010では「スコアの安定性に依存」を**制限事項**として記載済み:
> スコアの安定性に依存: QualityJudgeのスコアが不安定だと誤判定の可能性

これは設計上の制約として受け入れられており、追加対策の必要性が示されていない。

### 代替案

`scoreDirection`による履歴トレーサビリティが必要な場合は、`noiseThreshold`なしで単独実装可能。
