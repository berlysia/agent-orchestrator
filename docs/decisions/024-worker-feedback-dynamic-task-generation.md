...
実装済み（Phase 1）:
- TaskCandidate 型定義
- WorkerFeedback 拡張（patterns フィールド）
- Worker プロンプト拡張（構造化フィードバック出力指示）
- extractWorkerFeedback 関数
- generateTaskCandidates 関数
- Leader 統合（フィードバック抽出・タスク候補生成）
- CLI コマンド（candidates, approve, execute-candidates）

未実装（Phase 2+）:
- execute-candidates での実際のタスク実行
- LLM によるカテゴリ・優先度判定
- 自動実行可能条件の緩和
- スキル自動生成連携
