...
import { unwrapTaskId } from '../../src/types/branded.ts';
 ...
 * Result型のアサーション補助関数
 */
export const assertOk = <T, E>(result: Result<T, E>): asserts result is { ok: true; val: T } => {
  if (!result.ok) {
    const errStr =
...
export const assertErr = <T, E>(result: Result<T, E>): asserts result is { ok: false; err: E } => {
  if (result.ok) {
